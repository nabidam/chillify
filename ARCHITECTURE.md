...
Gate containment is enforced at two points, because no single point can see the whole picture. **On the host**, where real paths are known, `prepare.sh` creates a gate tree only beneath the repository's `.gate/` and refuses any name that resolves elsewhere, and `cleanup.sh` removes only a direct child of that tree. **In the process**, fixture provider implementations may bind only when `CHILLIFY_ENV=gate`, `CHILLIFY_GATE_ROOT` declares a containment root, `CHILLIFY_FIXTURE_ROOT` and both storage roots resolve beneath it, the two storage roots share one directory, and `CHILLIFY_REDIS_PREFIX` begins with `chillify:gate:`. Any mismatch fails startup before migration or Redis mutation.

The containment root is declared rather than derived from this repository's layout. A gate runs in the production containers, where the process sees `/var/lib/chillify` bind mounts and no repository at all, so a repository-relative rule is unsatisfiable there — and would not be worth satisfying: inside the container a gate and a household deployment present identical paths, so such a check would confirm nothing about what is really mounted. Which host directories are exposed is decided before the process starts, which is why that half of the guarantee lives in the scripts. The guarded gate-seed entry calls the production repositories/media services after API readiness; it is a data-fixture tool, not an alternate application composition. Production mode never imports or registers fixture adapters.
